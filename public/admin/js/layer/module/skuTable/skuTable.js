    ...
    class SkuTable {
        ...
        options = {
            ...
            multipleSkuTableConfig: {
                ...
                tbody: [
                    {type: 'image', field: 'goods_item_sku[picture]', value: '', verify: '', reqtext: ''},
                    {type: 'input', field: 'goods_item_sku[price]', value: '0', verify: '', reqtext: '销售价不能为空'},
                    {type: 'input', field: 'goods_item_sku[market_price]', value: '0', verify: '', reqtext: '市场价不能为空'},
                    {type: 'input', field: 'goods_item_sku[cost_price]', value: '0', verify: '', reqtext: '成本价不能为空'},
                    {type: 'input', field: 'goods_item_sku[stock]', value: '0', verify: '', reqtext: '库存不能为空'},
        ...
        constructor(options) {
            ...
            if (this.options.skuDataUrl) {
                Util.request.post({url: this.options.skuDataUrl, data: {item_id: this.data.productId, _method: 'sku'}}, (res) => {
        ...
        listen() {
            ...
            form.on('select(fairy-product-type)', function (data) {
                ...
                if (data.value === '') {
                    ...
                    Util.request.post({url: that.options.attrSpecUrl, data: {type_id: data.value, _method: 'attrspec'}}, (res) => {
                        that.resetRender([that.options.attributeTableElemId, that.options.specTableElemId, that.options.skuTableElemId]);
            ...
            $(document).on('click', `#${this.options.specTableElemId} .fairy-spec-create`, function () {
                layer.prompt({title: '规格'}, function (value, index, elem) {
                    Util.request.post(
                        {url: that.options.specCreateUrl, data: {title: value, type_id: that.data.productTypeId, _method: 'speccreate'}},
                        function (res) {
            ...
            $(document).on('click', `#${this.options.specTableElemId} .fairy-spec-value-create`, function () {
                ...
                layer.prompt({title: '规格值'}, function (value, index, elem) {
                    Util.request.post(
                        {url: that.options.specValueCreateUrl, data: {spec_id: specId, title: value, _method: 'specvalcreate'}},
                        function (res) {
                ...
                if (typeof $(this).attr('data-spec-id') !== "undefined") {
                    ...
                    if (that.options.specDeleteUrl) {
                        Util.request.post({
                            ...
                            data: {id: specId, type_id: that.data.productTypeId, _method: 'specdelete'}
                        }, function (res) {
                    ...
                    if (that.options.specValueDeleteUrl) {
                        Util.request.post({
                            ...
                            data: {id: specValueId, _method: 'specvaldelete'}
                        }, function (res) {
        ...
        renderFormItem(label, content, target) {
            var css = '';
            if(content.indexOf("<tbody></tbody>") != -1) css = 'style="display:none;"';
            var html = '';
            html += '<div class="layui-form-item" '+css+'>';
        ...
        renderIsAttribute(type) {
            var html = '';
            html += `<input type="radio" name="goods_item[is_attribute]" title="统一规格" value="0" lay-filter="fairy-is-attribute" ${type == '0' ? 'checked' : ''}>`;
            html += `<input type="radio" name="goods_item[is_attribute]" title="多规格" value="1" lay-filter="fairy-is-attribute" ${type == '1' ? 'checked' : ''}>`;
            this.renderFormItem('规格类型', html, this.options.isAttributeElemId);
        }

        renderProductType() {
            Util.request.post({url: this.options.productTypeUrl, data: {_method: 'itemtype'}}, (res) => {
                var html = '';
                html += `<select name="goods_item[product_type_id]" lay-filter="fairy-product-type" id="${this.options.productTypeElemId}">`;
                html += '<option></option>';
                ...
                if (this.data.productTypeId) {
                    // 获取商品类型下的规格和属性接口
                    Util.request.post({url: this.options.attrSpecUrl, data: {type_id: this.data.productTypeId, _method: 'attrspec'}}, (res) => {
        ...
        makeSkuName(sku, conf) {
            return 'goods_item_skus[' + (this.options.skuNameType === 0 ? sku.id : sku.title) + '][' + conf.field + ']';
        }
        ...
        getFormSkuData() {
            ...
            $.each(form.val(this.getFormFilter()), function (key, value) {
                if (key.startsWith('goods_item_skus')) {
                    skuData[key] = value;
                }
            });
