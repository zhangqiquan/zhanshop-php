            ...
            if (!$tabPane.html()) {
                if ($tabPane.is('iframe')) {
                    ...
                    if (!$tabPane.attr('src') || (!$tabPane.attr('src') && options.refresh == 'no') || (options.refresh == 'nav' && isNavBar) || options.refresh == 'all'){
                        ...
                        time = time.slice(0, 8);
                        // 如果一个url包含/ 并且不包含.
                        if(param.url.indexOf("/") != -1 && param.url.indexOf(".html") == -1){
                            var urlArr = param.url.split('/');
                            var pageName = urlArr[0];
                            urlArr.splice(0, 1);
                            var urlQuery = "";
                            if(window.accessQuery){
                                urlQuery = "&"+window.accessQuery;
                            }
                            param.url = pageName+'.html?_id='+ urlArr.join('/')+urlQuery+'&t='+time;
                            window.accessQuery = null;
                        }else{
                            param.url += '?t='+time;
                        }
