    ...
    ready(function(){
        layui.use(['zhanshop'], function(){
            ...
            function requestMenu(){
                zhanshop.ajax(API_ADDRESS+'/v1.0.0/apidoc', 'POST', {"_method":"apis", '_auth' : sign}, {}, function(res){
                    // 选项卡
                    $('#iframe-content').multitabs({
                        ...
                        init : [{
                            type : 'main',
                            title : '首页',
                            url : res.data.apptype+'main.html'
                        }]
                    });
                    try{
                        var menus = [];
                        var number = 1;
                        var itemNumber = 100000;
                        for(var i in res.data.menu){
                            menus.push({
                                "id" : number,
                                "name" : res.data.menu[i]['title'],
                                "pid" : 0,
                                "icon" : "mdi mdi-file-word",
                                "target" : "_self",
                                "url" : "",
                            });
                            var item = res.data.menu[i]['apis'];
                            for (var ii in item){
                                itemNumber++;
                                menus.push({
                                    "id" : itemNumber,
                                    ...
                                    "icon" : "",
                                    "target" : "",
                                    "url" : res.data.apptype+'.html?uri='+item[ii]['uri']+'&version='+item[ii]['version'],
                                });
                            }
                            number++;
                        }
                        console.log(menus);
                        var json_str = JSON.stringify(menus);
                        setSidebar(json_str);
                    }catch (e) {
                        console.error(e);
                    ...
                    if(xhr.responseJSON.code <= 10001){
                        sign = prompt("请输入访问密码?");
                        while (sign == ''){
                            sign = prompt("请输入访问密码?");
                        }
                        zhanshop.setcookie('_auth', sign);
                        return requestMenu();
                    }
