...
use app\api\admin\v4_0_0\service\common\Menu;
use zhanshop\App;
use zhanshop\Csv;
use zhanshop\Helper;
...
class BaseFinder
{
    ...
    public function rendertable(Request &$request, Response &$response){
    ...
    public function deletetable(Request &$request, Response &$response){
        $input = $request->post();
    ...
    public function searchfromtable(Request &$request, Response &$response){
        $data = [];
        foreach($this->menuData['table_names'] as $v){
            $include = $this->getCols($v);
    ...
    protected function getTableComment(string $dbName){
        $parseStrModel = Helper::parseStrModel($dbName);
    ...
    public function xmselecttable(Request &$request, Response &$response){
        $input = $request->validateData($request->get(), [
            'value_menu' => 'Required',
        ])->getData();
     ...
     * @return array
     */
    public function cascadertable(Request &$request, Response &$response){
        $input = $request->validateData($request->get(), [
            'value_menu' => 'Required',
        ])->getData();
